$(document).ready(function() {   
  window.setInterval(ForceControlInjection , 1000);
});
...
function ForceControlInjection() {
  if ($("#trackInfo").length > 0 && $("#downloadLink").length == 0) {  
    ...
    var artistName = fullArtistName.replace(byArtistText, '').trim().replace('.', '').replace(/[^a-z0-9\s&$]/gi, '');;
    var trackName = $("#trackInfo h3").text().trim().replace('.', '').replace(/[^a-z0-9\s&$]/gi, '');;
    ...
    var prettyMp3Link = mp3Link.replace(mp3ID, artistName +'-' + trackName)
        
    $("#trackInfo").append("<span id='downloadLink' class='p-fontHighlight trackinfo-p__span'>&#x25BC</span> <a href='" + prettyMp3Link + "' target='_blank'>Download this song</a>");        
